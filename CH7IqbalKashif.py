...
# This function Describes.. get numbers from user and append the number input to numsList through for loop, and return a List
def getNums():
    numsList = []
...
# This is function find a Lowest number, use a for loop iterating through  every element in numsList, Use if to check if the element is less then to  lo variable, then assign the list element to lo variable, and finally a return lowest number in lo variable
def findLo(numsList):
    lo = numsList[0]
    for num in numsList:
        if num<lo:
...
# findHi function  same as  work of the above function findLo but this find a Highest numbers in the list, using a if to check if element is greater then hi variable then assign the list element to hi variable
def findHi(numList):
    hi = numList[0]
    for num in numList:
        if num>hi:
...
# findTotal describes ,  all element sums of numslist  through for loop then assign total variable and finally return total vairable 
